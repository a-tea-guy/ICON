...
from algorithms.PCG import PCGradPretrain
...
from examples.algorithms.icon import RegDisent
from examples.algorithms.icon_noaug import RegDisentNoAug
from examples.algorithms.icon_det import RegDet
...
def initialize_algorithm(config, datasets, train_grouper, unlabeled_dataset=None):
    ...
    if config.algorithm == 'ERM':
        ...
        algorithm = PseudoLabel(
            ...
            metric=metric,
            n_train_steps=n_train_steps)
    elif config.algorithm == 'RegDNA':
        n_epochs = min(config.n_epochs, 2)
        n_train_steps = math.ceil(len(train_loader)/config.gradient_accumulation_steps) * n_epochs
        algorithm = RegDisentNoAug(
            config=config,
            d_out=d_out,
            grouper=train_grouper,
            loss=loss,
        ...
        if config.soft_pseudolabels:
            ...
            unlabeled_loss = loss
        algorithm = PCGradPretrain(
            config=config,
            d_out=d_out,
            unlabel_head=None,  # to be initialized later during update epoch
            grouper=train_grouper,
            loss=loss,
            unlabeled_loss=unlabeled_loss,
            metric=metric,
