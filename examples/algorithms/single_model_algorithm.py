...
class SingleModelAlgorithm(GroupAlgorithm):
    ...
    def __init__(self, config, model, grouper, loss, metric, n_train_steps):
        ...
        if self.pc_grad:
            scheduler = initialize_scheduler(config, self.optimizer.optimizer, n_train_steps)
        else:
    ...
    def _update(self, results, should_step=False):
        ...
        if not self.pc_grad:
            ...
            objective, losses = self.objective(results)
            results['objective'] = objective.item()
            self.optimizer.calculate_pc_grads(losses)
            if should_step:
