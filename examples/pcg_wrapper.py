...
try:
    idx = args.cmd.index("--exp_name")
    exp_name = args.cmd[idx + 1]
    args.cmd = (
        args.cmd[:idx] + args.cmd[idx + 2 :]
    )
except:
    exp_name = 'debug'  # default in run_expt.py

try:
    idx = args.cmd.index("--load_featurizer_only")
    args.cmd = (
        args.cmd[:idx] + args.cmd[idx + 2 :]
    )
except:
...
for i in range(1, args.num_iters + 1):
    ...
    if i == 1:
        cmd = (
            f"python {prefix}/run_expt.py --algorithm PCG {' '.join(args.cmd)}"
