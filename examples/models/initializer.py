...
def initialize_model(config, d_out, is_featurizer=False, load_all_overwrite=False):
    """
    Initializes models according to the config
        Args:
            ...
            - load_all_overwrite: if True, overwrite load_featurizer_only in configs to load featurizer and classifier
        Output:
    ...
    if config.pretrained_model_path is not None:
        ...
        if 'bert' not in config.model:  # We've already loaded pretrained weights for bert-based models using the transformers library
            try:
                if featurize:
                    if config.load_featurizer_only and (not load_all_overwrite):
